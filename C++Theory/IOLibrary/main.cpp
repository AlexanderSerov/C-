...
int main(int argc, char** argv)
{
    ...
    std::wstring name = L"Моё имя";
    std::wstring native = L"Native string";
    // wcout.imbue(std::locale("ru_RU.UTF-8"));             // Under Unix, but not tested
    _setmode(_fileno(stdout), _O_U16TEXT);                  // Under Windows. Switch console
    wcout << str << endl << name << endl << native << endl;*/

    /*wofstream fs(L"D:\\Repos\\CPlusPlus\\C++Theory\\IOLibrary\\Text.txt", ios::app);
    if (!fs.is_open()) {
        wcout << L"Failed to open Points.txt" << endl;
        return -1;
    }
    fs.imbue(std::locale("ru_RU.UTF-8"));
    wcout << wcslen(str);
    std::copy(str, str+wcslen(str), ostreambuf_iterator<wchar_t>(fs));*/
    
    // Vanin case        
    /*std::wifstream iFile("C:/Users/alexa/Desktop/allNames.txt");
    std::wofstream fs("D:\\Repos\\CPlusPlus\\C++Theory\\IOLibrary\\Text.txt", ios::app);
    fs << endl;
    
    std::copy_n(std::istream_iterator<std::wstring, wchar_t>(iFile), 10,
        std::ostream_iterator<std::wstring, wchar_t>(fs, L"\n"));
